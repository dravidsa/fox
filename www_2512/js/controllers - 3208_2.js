foxapp = angular.module('app.controllers', ["ion-datetime-picker"])
	...
	*/
	...
	var dt = $scope.strPad($scope.forDate.getDate(),2,0) + "-" + $scope.strPad(month,2,0)  + "-" + year ; 
	...
	var vehicleData = { vehicleId : $scope.vehicleId , fromDate : dt + " " + $scope.strPad($scope.fromTime.getHours(),2,0)+ ":"+ $scope.strPad($scope.fromTime.getMinutes(),2,0) , toDate : dt+ " " + $scope.strPad($scope.toTime.getHours(),2,0) + ":"+ $scope.strPad($scope.toTime.getMinutes(),2,0)  } ;
...
$scope.strPad = function(input, length, string) {
		console.log( " in strpad ");
...
$scope.getLocations  = function() { 
	...
	var month = $scope.forDate.getMonth() + 1 ; 
	var year = $scope.forDate.getYear() + 1900 ; 
	console.log( " month is now in getLoca " + $scope.strPad(month,2,0)  + "year is " + year )   ; 
	var dt = $scope.strPad($scope.forDate.getDate(),2,0)  + "-" + $scope.strPad(month,2,0)  + "-" + year ; 
	
	
	var vehicleData = { vehicleid : $scope.vehicleId.toUpperCase() , from_dt : dt + " " + $scope.strPad($scope.fromTime.getHours(),2,0)+ ":"+ $scope.strPad($scope.fromTime.getMinutes(),2,0) , to_dt : dt+ " " + $scope.strPad($scope.toTime.getHours(),2,0)+ ":"+ $scope.strPad($scope.toTime.getMinutes(),2,0) , status : 'specific' } ;
 ...
 */
	...
	console.log("in showdate"); 
	 ...
	 var ipObj1 = {
	  ...
	  $scope.forDate = $scope.strPad(dateArr[2],2,0) +'-'+$scope.strPad(dateArr[1],2,0)+'-'+$scope.strPad(dateArr[3],2,0) ; 
	  console.log( " forDate is "+ $scope.forDate) ; 
...
.controller('LiveCtrl', function( $scope, $rootScope, socket,  $localstorage , $ionicSlideBoxDelegate, $ionicLoading) {
...
socket.on( "gps", function(data) {
    console.log("here for gps " + data[0].vehicle_id ) ; 
 //var latlongArr = [] ;
  
    if ( (data[0].vehicle_id).toUpperCase() == ($scope.vehicleId).toUpperCase() ) {  
         var location = data[0] ; 
         console.log ( "got notification" + data[0].latitude +"xx" +  data[0].longitude) ; 
		 $scope.GPSMessage = "got " +  data[0].latitude  + "-" + data[0].longitude ; 
		latlongArr = $localstorage.getObject("latlongArr") ; 
		if ( latlongArr.length == 0 ) { 
