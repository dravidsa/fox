foxapp = angular.module('app.controllers', ["ion-datetime-picker"])
 ...
 $scope.ImageMessage = "Showing Images for Vehicle"; 
	...
	$scope.photos = Images.get() ; 
	console.log( "from service got " + $scope.photos.length ); 
	...
	for ( i = 0 ; i < data.length ; i ++ ) {
                      ...
                      "desc" :'' } ; 
							...
							console.log(new_image.src + " scope photos now has 1 " + imgArr.length);	
							 
							}
							*/
							
	//$scope.photos = Images.get() ; console.log( "got +++"+ $scope.photos.length) ; 
	//$scope.photos = $scope.imageArr  ; 
	/*
$scope.photos = Images.get() ; 
	  console.log( "got +++"+ $scope.photos.length) ; 
	  */
	  //$localstorage.setObject("imageArr", imgArr) ;
	}
	//set the scope here from return method of Images .
